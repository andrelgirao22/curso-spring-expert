...
public class FotoStorageS3 implements FotoStorage {
	...
	private static final String BUCKET = "truffle-app";
	...
	public String salvar(MultipartFile[] files) {
		...
		if(files != null && files.length > 0) {
			...
			try {
				...
				enviarFotoThumbnail(novoNome, arquivo, acl);
			} catch (IOException e) {
	...
	public byte[] recuperarFoto(String foto) {
		...
		try {
			...
			logger.error("Não conseguiu recuperar fotos do S3 " + e);
		}
	...
	public byte[] recuperarThumbnail(String foto) {
		return recuperarFoto(FotoStorage.THUMBNAIL_PREFIX + foto);
	}
	...
	public String getUrl(String foto) {
		if (!StringUtils.isEmpty(foto)) {
			return "https://s3-sa-east-1.amazonaws.com/truffle-app/" + foto;
		}
		
		return null;
	}
	
	private void enviarFotoThumbnail(String novoNome, MultipartFile arquivo, AccessControlList acl) throws IOException {
